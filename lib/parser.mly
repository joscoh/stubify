...
%token SIZEOF PTR INC DEC LEFT RIGHT LEQ GEQ EQEQ EQ NEQ LT GT
  ...
  AUTO REGISTER INLINE NORETURN CHAR COMPLEX SHORT INT LONG SIGNED UNSIGNED FLOAT DOUBLE
  UNDERSCORE_BOOL CONST VOLATILE VOID STRUCT UNION ENUM CASE DEFAULT IF ELSE
  SWITCH WHILE DO FOR GOTO IMAGINARY CONTINUE BREAK RETURN BUILTIN_VA_ARG ALIGNOF
...
| ctx = save_context LBRACE midrule({ Echo.set_printing();}) block_item_list? midrule({Echo.reset_printing();}) RBRACE
    { ctx() }

function_definition:
| ctx = function_definition1 compound_function_statement
    { ctx () }
